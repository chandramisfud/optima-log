...
export default function EmailContentPage() {
  ...
  return (
    <div style={{ padding: "20px", background: "white", width: "100%", height: "100%", overflowY: "auto" }}>
      ...
      <p style={{ marginBottom: "10px" }}>
        <strong>From:</strong>{" "}
        {emailContent.from_name ? `${emailContent.from_name} <${emailContent.from_email}>` : emailContent.from_email}
      </p>
      <p style={{ marginBottom: "10px" }}>
        <strong>To:</strong> {emailContent.to.map((recipient) => recipient.email).join(", ")}
      </p>
